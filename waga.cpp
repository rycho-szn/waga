...
void calibrate(HX711& scale) {
    std::cout << "Połóż produkt i podaj jego wagę (w gramach): ";
    float knownWeight;
    std::cin >> knownWeight;
    ...
    std::cout << "Kalibracja zakończona. Współczynnik wagi ustawiony na " << reading / knownWeight << std::endl;
}
...
void setGain(HX711& scale) {
    int gain;
    std::cout << "Podaj wartość gain (32, 64, 128): ";
    std::cin >> gain;
    scale.set_gain(gain);
    std::cout << "Gain ustawiony na " << gain << "." << std::endl;
}

void weighItems(const Recipe& recipe, HX711& scale) {
    for (const auto& ingredient : recipe.ingredients) {
        std::cout << "Ważenie " << ingredient.name << ". Wymagana waga: " << ingredient.weight << " gramów." << std::endl;
        float weight = 0;
        while (true) {
            weight = scale.get_units(10);
            std::cout << "Na wadze: " << weight << " gramów." << std::endl;
            ...
            if (weight < lower_bound) {
                ...
                std::cout << "Za dużo produktu " << ingredient.name << ". Proszę odjąć trochę produktu." << std::endl;
            } else {
        ...
        tare(scale); // Tarowanie wagi po zważeniu każdego składnika
    }
}
...
void weighSingleItem(HX711& scale) {
    ...
    std::cout << "Waga produktu wynosi: " << weight << " gramów." << std::endl;
}
